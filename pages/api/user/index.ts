...
const handler = async (req: NextApiRequest, res: NextApiResponse<IResponse>) => {
    switch (req.method) {
        ...
        case 'POST':
            ...
            const existingEmail = await userDao.findUser({ email: req.body.email }, {}, {});
            const existingUsername = await userDao.findUser({ username: req.body.username }, {}, {});

            if (existingEmail) return api400(req, res, 'Email already taken')
            if (existingUsername) return api400(req, res, 'Username already taken')
